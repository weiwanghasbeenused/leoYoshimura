...
// 	$sql = 'SELECT objects.* FROM objects, wires WHERE objects.active = "1" AND wires.active = "1" AND wires.toid = objects.id AND wires.fromid = "0" AND objects.name1 NOT LIKE ".%" AND objects.name1 NOT LIKE "\_%" AND objects.id = (SELECT min(id) FROM objects WHERE id > "' . $id . '" ) ORDER BY objects.begin DESC';
